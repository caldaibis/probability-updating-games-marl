...
class Model(ABC):
    @classmethod
    def create(cls, game: pu.Game):
        return cls._apply(cls._create_learn_env(game))

    @classmethod
    def _create_env(cls, game: pu.Game):
    ...
    @classmethod
    def _create_learn_env(cls, game: pu.Game):
        return ss.concat_vec_envs_v0(cls._create_env(game), 1, num_cpus=1, base_class='stable_baselines3')

    @classmethod
    def predict(cls, game: pu.Game, model_path: str) -> (np.ndarray, np.ndarray, np.ndarray, list):
